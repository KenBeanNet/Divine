        ...
        if (!var1.isRemote && var5 instanceof EntityLiving)
        {
            if (this.firetick == this.firemax && this.firemax != 0)
            {
                if (var5 instanceof EntityPlayerMP)
                {
                    ...
                    {
                        if (var7.dimension != DivineRPG.mythrilID)
                        {
                            var7.mcServer.getConfigurationManager().transferPlayerToDimension(var7, DivineRPG.mythrilID, new TeleporterMythril(var7.mcServer.worldServerForDimension(DivineRPG.mythrilID)));
                        }
                        else
                        {
                            var7.mcServer.getConfigurationManager().transferPlayerToDimension(var7, 0, new TeleporterMythril(var7.mcServer.worldServerForDimension(0)));
                        }
                    }
                }
